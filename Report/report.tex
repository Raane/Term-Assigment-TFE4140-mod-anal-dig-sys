...
\begin{titlepage}
    \vspace*{2cm}
    \begin{abstract}
The norwegian univercity of Science and Technology is going to launch a student satelite.
This paper explore a solution with a liaison module for the satelite that will handle multiple redundant computing systems onboard the satelite.
This redundancy will help increase the expected lifetime of the satelite by allowing the system to remain fully functional even with one or more systems failed.
The system will also ensure error correction and detection on transmitted data.
This is achived through hammingcodes handeling one bit error correction and two bit error detection.
    \end{abstract}
\end{titlepage}
...
\break
This report presents the solution for the Liasion system designed by group 1. The problem and the requirements are described in the Term Assignment\cite{assignment-text} for the Course TFE4140 Modelling and Analysis of Digital Systems
Chapter 2 presents the problem and all required functionality in details.
Chapter 3 presents the technical solutions to the problem, split into different sub-problems similarly to the Term Assignment paper\cite{assignment-text}.
...
The goal of this assignment\cite{assignment-text} is to design a fault tolerant system called Liasion, which exploits redundancy.
...
This problem is to be solved by implementing a module with Error Correcting Circuit (ECC) that uses Hamming Code\cite{hamming-codes}.
...
The description that follows is directly specified in the Term Assignment\cite{assignment-text}.
\begin{figure}[h!]
    \centering
...
\begin{itemize}
    \item \textbf{February 18th, 2014}: Assignment 3 \todo{ INSERT CITATIon TO ASSIGNMENT 3 here}. of the Course, where the individual One-Bit Voters are designed. They serve as basis of the 8-bit voters
...
This chapter will explain how the group has implemented the solution for the Term Assignment\cite{assignment-text}.
The Term Assignment\cite{assignment-text} was split into 5 sub-problems, therefore this report will detail the solution to each sub-problem accordingly.
